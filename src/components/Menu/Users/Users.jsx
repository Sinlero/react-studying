        ...
        <div>
            <div className={style.pagination}>
                <Pagination current={props.currentPage} pageSize={props.pageSize} total={props.totalRecords}
                            onChange={props.onPageClick} onShowSizeChange={props.onChangePageSize}
                            showSizeChanger={true} pageSizeOptions={[2, 3, 4]}/>
            </div>

            {props.users.map(user =>
                <div className={style.userItem} key={user.id}>
                    <NavLink to={"/profile/" + user.id}>
                        <Avatar size={64} src={user.photos !== null ? user.photos : reactLogo}
                                     alt="avatar"/>
                    </NavLink>
                    ...
                    {user.followed
                        ? <Button type="primary" onClick={() => props.unfollow(user.id)}
